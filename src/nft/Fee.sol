...
library Fee {
    ...
    function receiveUSDC(address from, uint256 value, address token, bytes memory authorizationParams) internal {
        IUSDC USDCToken = IUSDC(token);
        require(USDCToken.balanceOf(from) >= value, "Insufficient balance");

        if (authorizationParams.length == 0 || keccak256(authorizationParams) == keccak256("")) {
            ...
            (uint256 validAfter, uint256 validBefore, bytes32 nonce, bytes memory signature) =
                abi.decode(authorizationParams, (uint256, uint256, bytes32, bytes));

            USDCToken.receiveWithAuthorization(from, address(this), value, validAfter, validBefore, nonce, signature);
        }
