...
contract Image {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory imgSVG = generateSVG(
            ...
            tokenLockStatus
                ? "??"
                : Strings.toString(nft.getStat(statLabels[0], tokenId)),
            tokenLockStatus
                ? "??"
                : Strings.toString(nft.getStat(statLabels[1], tokenId)),
            tokenLockStatus
                ? "??"
                : Strings.toString(nft.getStat(statLabels[2], tokenId)),
            tokenLockStatus
                ? "??"
                : Strings.toString(powerLevel(tokenId, msg.sender)),
            tokenLockStatus
